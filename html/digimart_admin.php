<?php
    ...
    if(isset($_POST['btnSubmit'])){
        ...
        $sql = "INSERT INTO `admin`(`id`, `first_name`, `last_name`, `email`) VALUES ('{$userId}','{$firstName}','{$lastName}','{$email}')";
    ...
    if(isset($_GET['delete'])){
        ...
        if ($result) {
            $sql = "UPDATE `admin` SET `is_deleted`= 1 WHERE `email` = '{$deleteEmail}'";
            mysqli_query($conn, $sql);
            
            header('Location: digimart_admin.php');
        }
    }
...
<head>
    <!-- title -->
	<title>Administrator | DigiMart</title>
...
<body>
    ...
    <div class="container">
        ...
        <div class="content p-1 mb-5 rounded-lg shadow-lg <?php if(isset($_COOKIE['theme']) && ($_COOKIE['theme']=='dark')) echo "bg-dark"; ?>">
            <h4 class="text-danger mb-3"><i class="fas fa-user-tie"></i> Administrator</h4>
                        ...
                        <div class="row mw-100 p-2" id="product-container">
                            ...
                            <div class="col-md-6 col-sm-12">
                                <div class="custom-control custom-checkbox">
                                    <form action="digimart_admin.php" method="post">
                                        <div class="">

                                            <?php

                                                $queryAdId = "SELECT * FROM `admin` ORDER BY `id` DESC LIMIT 1";
                                                ...
                                                if (mysqli_num_rows($result_set) == 1) {
                                                    ...
                                                    $lastNum = substr($lastId['id'],3,strlen($lastId['id']));
            ...
            <table class="table table-striped <?php if(isset($_COOKIE['theme']) && ($_COOKIE['theme']=='dark')) echo "text-white table-dark"; ?>">
                ...
                <tbody id="adminList">
                    ...
                    <tr>
                        ...
                        <td>
                            <?php 
                                if($_SESSION['digimart_current_user_id'] != $row['id']) {
                            ?>
                            <a href="digimart_admin.php?delete=<?php echo $row['email']; ?>" class="text-danger" onclick="return confirm('This action will remove this admin account from the system?.');" ><i class="far fa-trash-alt"></i></a>
                            <?php
                                }
                            ?>
                        </td>
