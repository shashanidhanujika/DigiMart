<?php
    ...
    if(isset($_POST['btnSubmit'])){
        ...
        $sql = "INSERT INTO `customer`(`id`, `first_name`, `last_name`, `email`) VALUES ('{$userId}','{$firstName}','{$lastName}','{$email}')";
        
        $result = mysqli_query($conn, $sql);

        if ($result) {
            $sql = "INSERT INTO `user`(`username`, `password`, `role`) VALUES ('{$email}','".md5("123")."','customer')";
    ...
    if(isset($_GET['delete'])){
        ...
        if ($result) {
            $sql = "UPDATE `customer` SET `is_deleted`= 1 WHERE `email` = '{$deleteEmail}'";
            mysqli_query($conn, $sql);
            
            header('Location: digimart_customer.php');
        }
    }
...
<body>
    ...
    <div class="container">
        ...
        <div class="content p-1 mb-5 rounded-lg shadow-lg <?php if(isset($_COOKIE['theme']) && ($_COOKIE['theme']=='dark')) echo "bg-dark"; ?>">
            <h4 class="text-danger mb-3"><i class="far fa-user"></i> Customer</h4>
                        ...
                        <div class="row mw-100 p-2" id="product-container">
                            ...
                            <div class="col-md-6 col-sm-12">
                                <div class="custom-control custom-checkbox">
                                    <form action="digimart_customer.php" method="post">
                                        <div class="">

                                            <?php

                                                $queryIoId = "SELECT * FROM `customer` ORDER BY `id` DESC LIMIT 1";
                                                ...
                                                if (mysqli_num_rows($result_set) == 1) {
                                                    ...
                                                    $lastNum = substr($lastId['id'],1,strlen($lastId['id']));
