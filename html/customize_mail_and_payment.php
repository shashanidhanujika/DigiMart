<?php
    ...
    if(isset($_GET['buy'])) {
        ...
        $queryAdId = "SELECT * FROM `customize_order` ORDER BY `id` DESC LIMIT 1";

        $result_set = mysqli_query($conn,$queryAdId);
        ...
        $orderInsertQuery = "INSERT INTO `customize_order`(`id`, `customer_id`, `unit_price`, `quantity`) VALUES ({$customizeId}, '{$_SESSION['digimart_current_user_id']}', {$_SESSION['total']}, {$_SESSION['customizeProductQuantity']})";
        
        $resultOrderInsert = mysqli_query($conn, $orderInsertQuery);
        
        if ($resultOrderInsert) {
            while ($i < $_SESSION['itemCount']) {

                $productInsertQuery = "INSERT INTO `customize_order_product`(`customize_order_id`, `product_id`) VALUES ({$customizeId}, {$_SESSION['productId'][$i]})";
        ...
        $_SESSION['total'] = null;
        $_SESSION['customizeProductQuantity'] = null;
                ...
                <div class="shadow-lg p-4 mb-4 rounded-lg col-lg-4 h-100 <?php if(isset($_COOKIE['theme']) && ($_COOKIE['theme']=='dark'))echo "text-white bg-dark"; ?>">
                    ...
                    <div id="orderSummeryDiv">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="p-2">
                                <input type="number" class="form-control form-control-sm mx-2 <?php if(isset($_COOKIE['theme']) && ($_COOKIE['theme']=='dark'))echo "text-white"; ?>" value="<?php echo $_SESSION['customizeProductQuantity']; ?>" readonly>
                            </div>
                        </div>

                        <div class="d-flex border-top mt-4 border-danger">
                            <div class="mr-auto p-2"><h5>Total</h5></div>
                            <div class="p-2"><h5>LKR <?php echo number_format($_SESSION['customizeProductQuantity']*$_SESSION['total'],2); ?></h5></div>
                        </div>
                    </div>
                    ...
                    <a onclick="window.location.href = 'customize_mail_and_payment.php?buy=1';" id="buy" name="buy" class="btn btn-danger w-100 mt-3">Buy</a>
